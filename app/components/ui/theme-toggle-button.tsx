...
export default function ThemeToggleButton({
  ...
  // Determine if we should show dark or light icon
  const showDarkIcon = React.useMemo(() => {
  ...
  return (
    ...
    >
      <SunIcon className={`size-[1.2rem] transition-all ${
        showDarkIcon ? 'rotate-90 scale-0' : 'rotate-0 scale-100'
      }`} />
      <MoonIcon className={`absolute size-[1.2rem] transition-all ${
        showDarkIcon ? 'rotate-0 scale-100' : '-rotate-90 scale-0'
      }`} />
